document.addEventListener("DOMContentLoaded", function () {
    ...
    function handleMouseDown(event) {
        ...
        if (element.id) {
            ...
            if (split[0] === "target" && split[1] !== "eye" && event.button === 0) {
                Events.Call("selectTarget", split[2]);
                targetLabel.textContent = "";
            }
        }
        if (event.button === 2) {
            leftTarget();
            Events.Call("leftTarget");
        }
    }
    ...
    Events.Subscribe("openTarget", openTarget);
    Events.Subscribe("closeTarget", closeTarget);
    Events.Subscribe("foundTarget", foundTarget);
    Events.Subscribe("validTarget", validTarget);
    Events.Subscribe("leftTarget", leftTarget);
