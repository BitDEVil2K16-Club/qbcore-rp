const InventoryContainer = Vue.createApp({
    ...
    methods: {
        ...
        closeInventory() {
            ...
            Events.Call("CloseInventory", inventoryName);
        },
        clearTransferAmount() {
        ...
        handleDropOnInventoryContainer() {
            if (this.isOtherInventoryEmpty && this.dragStartInventoryType === "player") {
                ...
                Events.Call("DropItem", {
                    ...newItem,
                    fromSlot: this.currentlyDraggingSlot,
                });
                Events.Subscribe("DropItemResponse", (response, item) => {
                    if (response) {
                        this.otherInventory[1] = item;
                        const draggingItemKey = Object.keys(this.playerInventory).find((key) => this.playerInventory[key] === item);
                        if (draggingItemKey) {
                            delete this.playerInventory[draggingItemKey];
                        }
                        this.otherInventoryName = response;
                        this.otherInventoryLabel = response;
                        this.isOtherInventoryEmpty = false;
                        this.clearDragData();
                    } else {
        ...
        handlePurchase(targetSlot, sourceSlot, sourceItem, transferAmount) {
            ...
            Events.Subscribe("AttemptPurchaseResponse", (response, data) => {
                if (response) {
        ...
        dropItem(item, quantity) {
            if (item && item.name) {
                ...
                if (playerItemKey) {
                    ...
                    Events.Subscribe("DropItemResponse", (response) => {
                        if (response) {
        ...
        useItem(item) {
            ...
            if (playerItemKey) {
                ...
                Events.Subscribe("UseItemResponse", (response, itemData) => {
                    if (response) {
        ...
        giveItem(item, quantity) {
            if (item && item.name) {
                ...
                if (playerHasItem) {
                    ...
                    Events.Call("GiveItem", {
                        item: selectedItem,
                        amount: amountToGive,
                    });

                    Events.Subscribe("GiveItemResponse", (response, data) => {
                        if (response) {
                            const amountToGive = data.amount;
        ...
        openWeaponAttachments() {
            ...
            if (!this.showWeaponAttachments) {
                ...
                Events.Call("GetWeaponData", {
                    weapon: this.selectedWeapon.name,
                    ItemData: this.selectedWeapon,
                });

                Events.Subscribe("GetWeaponDataResponse", (response) => {
                    if (response.data) {
                        ...
                        console.error("An error occurred while getting weapon data:", response.error);
                    }
                });
            } else {
        ...
        removeAttachment(attachment) {
            ...
            Events.Call("RemoveAttachment", {
                AttachmentData: attachment,
                WeaponData: this.selectedWeapon,
            });

            Events.Subscribe("RemoveAttachmentResponse", (response) => {
                if (response.data) {
                    ...
                    console.error("An error occurred while removing the attachment:", response.error);
    ...
    mounted() {
        Events.Subscribe("openInventory", (data) => {
        ...
        Events.Subscribe("updateInventory", (data) => {
            this.updateInventory(data);
        });
        Events.Subscribe("toggleHotbar", (data) => {
            this.toggleHotbar(data);
        });
        Events.Subscribe("itemBox", (data) => {
            this.showItemNotification(data);
        });
        Events.Subscribe("requiredItem", (data) => {
            this.showRequiredItem(data);
        });
        Events.Subscribe("purchaseResult", (canPurchase) => {
            this.handlePurchase(canPurchase);
        });
    },
    beforeDestroy() {
