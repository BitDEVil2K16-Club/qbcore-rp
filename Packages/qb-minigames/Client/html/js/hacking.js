...
Events.Subscribe("KeyPress", function (key_name) {
    ...
    if (gameStarted && key === "BackSpace") {
        gameStarted = false;
        Events.Call("hackClosed");
        resetGameState();
        return;
    }
...
Events.Subscribe("startHack", function (solutionsize, timeout) {
    if (!gameStarted) {
        hackingContainer.style.display = "flex";
        startGame(solutionsize, timeout);
    }
});
