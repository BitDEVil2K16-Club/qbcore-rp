...
const endGame = (status) => {
    Events.Call("skillbarFinish", status);
    resetSkillbar();
};

Events.Subscribe("KeyPress", function (key_name) {
    if (config.validKeys.includes(key_name)) {
        const d_start = radiansToDegrees(gStart);
        const d_end = radiansToDegrees(gEnd);
        if (key_name === keyToPress && degrees >= d_start && degrees <= d_end) {
            streak += 1;
            if (streak === needed) {
                endGame(true);
            } else {
...
Events.Subscribe("openSkillbar", function (difficulty, validKeys) {
