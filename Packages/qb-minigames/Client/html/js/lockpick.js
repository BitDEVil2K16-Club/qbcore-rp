...
const unlock = () => {
    closeLockpick();
    Events.Call("lockpickFinish", true);
};
...
const outOfPins = () => {
    closeLockpick();
    Events.Call("lockpickFinish", false);
};
...
Events.Subscribe("KeyDown", (key_name) => {
    let action = keyActionMap[key_name];
    if (action && !userPushingCyl && !gameOver && !gamePaused) {
        action();
    }
});

Events.Subscribe("KeyUp", (key_name) => {
    let action = keyActionMap[key_name];
    if (action && !gameOver) {
        unpushCyl();
    }
...
Events.Subscribe("startLockpick", function (pins) {
