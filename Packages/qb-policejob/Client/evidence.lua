...
Events.SubscribeRemote('evidence:client:SetStatus', function(statusId, time)
...
Events.SubscribeRemote('evidence:client:AddBlooddrop', function(bloodId, citizenid, bloodtype, coords)
    Blooddrops[bloodId] = {
        citizenid = citizenid,
        bloodtype = bloodtype,
        coords = {
...
Events.SubscribeRemote('evidence:client:RemoveBlooddrop', function(bloodId)
    Blooddrops[bloodId] = nil
    CurrentBlooddrop = 0
end)

Events.SubscribeRemote('evidence:client:AddFingerPrint', function(fingerId, fingerprint, coords)
...
Events.SubscribeRemote('evidence:client:RemoveFingerprint', function(fingerId)
    Fingerprints[fingerId] = nil
    CurrentFingerprint = 0
end)

Events.SubscribeRemote('evidence:client:ClearBlooddropsInArea', function()
    ...
    QBCore.Functions.Progressbar('clear_blooddrops', Lang:t('progressbar.blood_clear'), 5000, false, true, {
        ...
        if Blooddrops and next(Blooddrops) then
            ...
            Events.CallRemote('evidence:server:ClearBlooddrops', blooddropList)
...
Events.SubscribeRemote('evidence:client:AddCasing', function(casingId, weapon, coords, serie)
    Casings[casingId] = {
        type = weapon,
...
Events.SubscribeRemote('evidence:client:RemoveCasing', function(casingId)
    Casings[casingId] = nil
    CurrentCasing = 0
end)

Events.SubscribeRemote('evidence:client:ClearCasingsInArea', function()
    local pos = GetEntityCoords(PlayerPedId())
    local casingList = {}
    QBCore.Functions.Progressbar('clear_casings', Lang:t('progressbar.bullet_casing'), 5000, false, true, {
        ...
        if Casings and next(Casings) then
            ...
            Events.CallRemote('evidence:server:ClearCasings', casingList)
    ...
    if key_name == 'RightMouseButton' then
        ...
        if next(Casings) then
            for k, v in pairs(Casings) do
                local dist = #(ped_coords - vector3(v.coords.x, v.coords.y, v.coords.z))
                if dist < 1.5 then
                    CurrentCasing = k
                end
            end
